...
function saveWateringData(data) {
  try {
    ...
    console.error("❌ Nepavyko išsaugoti watering.json:", err.message);
  }
}
...
function updatePlantDays() {
  ...
  if (wateringData.lastUpdate !== today) {
    for (const userId in wateringData) {
      ...
      for (const house in wateringData[userId]) {
        wateringData[userId][house].plantDays += 1;
      }
    }
...
function decreaseWateringLevels() {
  ...
  if (!wateringData.lastDecreaseTime || now - wateringData.lastDecreaseTime >= 60 * 60 * 1000) {
    ...
    for (const userId in wateringData) {
      if (["lastUpdate", "lastMessageId", "lastDecreaseTime", "customSections"].includes(userId)) continue;
      for (const house in wateringData[userId]) {
        if (wateringData[userId][house].percent > 0) {
          wateringData[userId][house].percent -= 4;
        }
      }
    }
    saveWateringData(wateringData);
...
client.once('ready', async () => {
  ...
  setInterval(async () => {
    ...
    for (const owner in grouped) {
      const extra = wateringData.customSections?.[owner] || "";
      embed.addFields({
        name: `🏡 ${owner} namai:`,
        value: (extra ? `📝 ${extra}\n` : "") + grouped[owner].join("\n"),
        inline: false
